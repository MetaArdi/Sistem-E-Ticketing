...
// Helper Function untuk Kirim Email E-Ticket PDF Resmi
if (!function_exists('sendTicketEmailDirect')) {
    function sendTicketEmailDirect($conn, $token_qr) {
        if (empty($token_qr)) return false;
        ...
        $stmt->bind_param("s", $token_qr);
        $stmt->execute();
        $res = $stmt->get_result();
        if ($res && $row = $res->fetch_assoc()) {
            $email_pembeli = $row['email_pembeli'];
            $nama_pembeli = $row['nama_pembeli'];
            $judul_event = $row['judul'];
            $pdf_url = (defined('BASE_URL') ? BASE_URL : 'http://localhost/') . "user/download_tiket.php?token=" . urlencode($token_qr);

            $to = $email_pembeli;
            $subject = "E-Ticket Resmi: " . $judul_event . " - HaloTiket";

            $message = "<!DOCTYPE html><html lang='id'><head><meta charset='UTF-8'></head><body style='font-family: Arial, sans-serif; background-color: #f8fafc; padding: 30px; margin: 0; color: #1e293b;'><div style='max-width: 520px; margin: 0 auto; background: #ffffff; border-radius: 20px; border: 1px solid #e2e8f0; overflow: hidden; box-shadow: 0 10px 25px rgba(0,0,0,0.05);'><div style='background: #0f1c3f; color: #ffffff; padding: 25px; text-align: center;'><h1 style='margin: 0; font-size: 24px; color: #00c2cb;'>HaloTiket</h1><p style='margin: 5px 0 0 0; font-size: 12px; color: #94a3b8;'>E-Ticket Resmi Pembelian Event</p></div><div style='padding: 30px;'><h2 style='margin-top: 0; color: #0f172a; font-size: 18px;'>Halo, " . htmlspecialchars($nama_pembeli) . "! 👋</h2><p style='font-size: 14px; color: #475569; line-height: 1.6;'>Terima kasih telah melakukan pemesanan tiket <strong>" . htmlspecialchars($judul_event) . "</strong> di HaloTiket.</p><p style='font-size: 14px; color: #475569; line-height: 1.6;'>Pembayaran Anda telah berhasil kami verifikasi <strong>LUNAS</strong>. Silakan unduh E-Ticket PDF Anda melalui tombol di bawah ini:</p><div style='text-align: center; margin: 30px 0;'><a href='$pdf_url' style='background-color: #00c2cb; color: #ffffff; padding: 14px 28px; text-decoration: none; border-radius: 12px; font-weight: bold; font-size: 14px; display: inline-block;'>Unduh E-Ticket (PDF)</a></div><div style='background: #f1f5f9; border-left: 4px solid #00c2cb; padding: 12px 16px; border-radius: 8px; font-size: 12px; color: #64748b;'>📌 <strong>Petunjuk Penting:</strong> Simpan E-Ticket ini dan tunjukkan file PDF / QR Code kepada petugas di lokasi acara saat check-in.</div></div><div style='background: #f8fafc; padding: 15px; text-align: center; font-size: 11px; color: #94a3b8;'>&copy; " . date('Y') . " HaloTiket. All rights reserved.</div></div></body></html>";
            ...
            $m_sent = @mail($to, $subject, $message, $headers, "-f no-reply@halotiket.com");
            if (!$m_sent) {
